...
async function putOnWrappedERC20TokensInCSTContract(addressContractERC20, addressUser, amountTokens) {
    try {
      ...
      var receipt = await Server.CSTContract.methods.putOnWrappedERC20TokensInCSTContract(addressContractERC20, addressUser, amountTokens)
      .send({from:creatorCSTAddress, gas:4000000});
...
async function withdrawWrappedERC20TokensInCSTContract(addressContractERC20, addressUser, amountTokens) {
    try {
        Server.web3.eth.accounts.wallet.add(privateKey);
        return await Server.CSTContract.methods.withdrawWrappedERC20TokensInCSTContract(addressContractERC20, addressUser, amountTokens)
        .send({ from: creatorCSTAddress, gas:4000000})
        .on("receipt", function (receipt) {
          if (receipt.status != true) {
            return false;
...
async function transferWrappedERC20TokensToAnotherAddressInCSTContract(addressContractERC20, addressSender, addressReceiver, amountTokens) {
  try {
      Server.web3.eth.accounts.wallet.add(privateKey);
      var receipt = await Server.CSTContract.methods.transferWrappedERC20TokensToAnotherAddressInCSTContract(addressContractERC20, addressSender, addressReceiver, amountTokens)
      .send({from:creatorCSTAddress, gas:4000000});
...
async function transferWrappedERC20TokensFromCSTtoUsers(addressContractERC20, addressReceiver, amountTokens) {
  try {
      Server.web3.eth.accounts.wallet.add(privateKey);
      var receipt = await Server.CSTContract.methods.transferWrappedERC20TokensFromCSTtoUsers(addressContractERC20, addressReceiver, amountTokens)
      .send({from:creatorCSTAddress, gas:4000000});
...
async function emitBetCreate(gameID, addressUser, addressContractERC20, amountTokens) {
  try {
      Server.web3.eth.accounts.wallet.add(privateKey);
      return await Server.CSTContract.methods.emitBetCreate(gameID, addressUser, addressContractERC20, amountTokens)
      .send({ from: creatorCSTAddress, gas:4000000 })
      .on("receipt", function (receipt) {
        if (receipt.status != true) {
          return false;
      ...
      .on("error", function (error) {
        console.log('tx_error_emitBetCreate = ', error);
        return false;
      });
  } catch (error) {
    console.log('try/catch_error_emitBetCreate = ', error);
    return false;
  }
}
...
async function emitBetJoin(gameID, addressUser, addressContractERC20, amountTokens) {
    try {
        Server.web3.eth.accounts.wallet.add(privateKey);
        return await Server.CSTContract.methods.emitBetJoin(gameID, addressUser, addressContractERC20, amountTokens)
        .send({ from: creatorCSTAddress, gas:4000000 })
        .on("receipt", function (receipt) {
          if (receipt.status != true) {
            return false;
        ...
        .on("error", function (error) {
          console.log('tx_error_emitBetJoin = ', error);
          return false;
        });
    } catch (error) {
      console.log('try/catch_error_emitBetJoin = ', error);
      return false;
    }
}

async function emitRefundTokens(addressUserLostConnection, addressUser1, addressContractERC20User1, amountTokensUser1, addressUser2, addressContractERC20User2, amountTokensUser2) {
    try {
        Server.web3.eth.accounts.wallet.add(privateKey);
        return await Server.CSTContract.methods.emitRefundTokens(addressUserLostConnection, addressUser1, addressContractERC20User1, amountTokensUser1, addressUser2, addressContractERC20User2, amountTokensUser2) // if successfull - return 'true'
        .send({ from: creatorCSTAddress, gas:4000000 })
        .on("receipt", function (receipt) {
          if (receipt.status != true) {
            return false;
        ...
        .on("error", function (error) {
          console.log('tx_error_emitRefundTokens = ', error);
          return false;
        });
    } catch (error) {
      console.log('try/catch_error_emitRefundTokens = ', error);
      return false;
    }
}
...
async function emitBetFinish(gameID, addressWinner, addressLoser) {
    try {
        Server.web3.eth.accounts.wallet.add(privateKey);
        return await Server.CSTContract.methods.emitBetFinish(gameID, addressWinner, addressLoser)
        .send({ from: creatorCSTAddress, gas:4000000 })
        .on("receipt", function (receipt) {
          if (receipt.status != true) {
            return false;
        ...
        .on("error", function (error) {
          console.log('tx_error_emitBetFinish = ', error);
          return false;
        });
    } catch (error) {
      console.log('try/catch_error_emitBetFinish = ', error);
      return false;
    }
}
...
async function stopContract() {
    try {
        Server.web3.eth.accounts.wallet.add(privateKey);
        return await Server.CSTContract.methods.stopContract()
        .send({ from: creatorCSTAddress })
        .on("receipt", function (receipt) {
          if (receipt.status != true) {
            return false;
          }
...
async function startContract() {
    try {
        Server.web3.eth.accounts.wallet.add(privateKey);
        return await Server.CSTContract.methods.startContract()
        .send({ from: creatorCSTAddress })
        .on("receipt", function (receipt) {
          if (receipt.status != true) {
            return false;
...
async function getBalanceCST(addressContractERC20, addressUser) {
    try {
        return await Server.CSTContract.methods.getBalanceCST.call(addressContractERC20, addressUser)
        .send({ from: creatorCSTAddress })
        .on("receipt", function (receipt) {
          if (receipt.receipt.status != true) {
            return false;
...
exports.emitBetJoin = emitBetJoin;
